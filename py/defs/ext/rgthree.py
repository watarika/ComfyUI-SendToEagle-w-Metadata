...
def get_width(dimensions, input_data):
    # dimensionsは"width x height  (type)"の形式であることを想定
    return dimensions.split("x")[0].strip()


def get_height(dimensions, input_data):
    # dimensionsは"width x height  (type)"の形式であることを想定
    return dimensions.split("x")[1].strip().split("(")[0].strip()
...
CAPTURE_FIELD_LIST = {
    ...
    "SDXL Empty Latent Image (rgthree)": {
        MetaField.IMAGE_WIDTH: {"field_name": "dimensions", "format": get_width},
        MetaField.IMAGE_HEIGHT: {"field_name": "dimensions", "format": get_height},
    },
}
